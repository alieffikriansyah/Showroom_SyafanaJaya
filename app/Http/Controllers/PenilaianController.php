...
class PenilaianController extends Controller
{
    ...
    public function tambah_penilaian(Request $request)
    {
        ...
        try {
            ...
            WHERE karyawan.id_karyawan = ' .  $request->karyawan;
            
            $karyawan = DB::select($query);
            \App\log::create([
                'user_id_user' => Auth::user()->id,
                'aksi' => 'Tambah Penilaian '. $karyawan[0]->name,
                'fitur' => 'penilaian'
            ]);
    ...
    public function ubah_penilaian_nilai(Request $request)
    {
        ...
        try {
            ...
            $penilaianUpdate = Penilaian::find($id_penilaian_nilai);
            // dd($penilaianUpdate);
            ...
            FROM karyawan
            INNER JOIN users ON karyawan.user_id_user = users.id
            WHERE karyawan.id_karyawan = ' . $penilaianUpdate->karyawan_id_karyawan;
            
            $karyawan = DB::select($query);
            // dd($karyawan);
            \App\log::create([
                'user_id_user' => Auth::user()->id,
                'aksi' => 'Update Nilai Skor Penilaian '. $karyawan[0]->name,
                'fitur' => 'penilaian'
            ]);
