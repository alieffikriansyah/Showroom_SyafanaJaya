...
class PenjualanController extends Controller
{
    ...
    public function getPenjualanPerBulan(Request $request) {
        ...
        $penjualanBulanan = Penjualan::select(
                ...
                DB::raw("SUM(harga * unit) as omzet"),
                DB::raw("SUM(unit) as total_unit")
            )
        ...
        $maxMonth = max($currentMonth, $penjualanBulanan->keys()->max() ?? 1);

        for ($m = 1; $m <= $maxMonth; $m++) {
            $labels[] = $monthNames[$m];
            $dataOmzet[] = isset($penjualanBulanan[$m]) ? (float)$penjualanBulanan[$m]->omzet : 0;
            $dataUnit[] = isset($penjualanBulanan[$m]) ? (int)$penjualanBulanan[$m]->total_unit : 0;
        }

        // Breakdown by Brand
        $brandData = Penjualan::select(
                'merk',
                DB::raw("SUM(harga * unit) as omzet"),
                DB::raw("SUM(unit) as total_unit")
            )
        ...
        foreach ($brandData as $b) {
            $brandLabels[] = ucfirst($b->merk);
            $brandUnits[] = (int)$b->total_unit;
            $brandOmzets[] = (float)$b->omzet;
        }

        // Summary
        $totalOmzet = array_sum($dataOmzet);
        $totalUnit = array_sum($dataUnit);
        $avgOmzet = count($dataOmzet) > 0 ? $totalOmzet / count($dataOmzet) : 0;

        $arr = [
            'labels' => $labels,
            'data' => $dataOmzet,
            'data_omzet' => $dataOmzet,
            'data_unit' => $dataUnit,
            'brands' => [
                'labels' => $brandLabels,
                'units' => $brandUnits,
            ...
            'summary' => [
                ...
                'avg_omzet_formatted' => 'Rp ' . number_format($avgOmzet, 0, ',', '.')
            ]
        ];
