...
class PengajuanCutiController extends Controller
{
    ...
    public function terima_pengajuanCuti(Request $request)
    {
        ...
        try {
            ...
            FROM karyawan
            INNER JOIN users ON karyawan.user_id_user = users.id
            WHERE karyawan.id_karyawan = ' . $PengajuanCuti->karyawan_id_karyawan;
            
            $karyawan = DB::select($query);

            // dd($PengajuanCuti->karyawan_id_karyawan);
            // dd($karyawan);
            \App\log::create([
                'user_id_user' => Auth::user()->id,
                'aksi' => 'Terima Pengajuan Cuti untuk karyawan ' . $karyawan[0]->name,
                'fitur' => 'pengajuanCuti'
            ]);
            \App\log::create([
                'user_id_user' => Auth::user()->id,
                'aksi' => 'Tambah Presensi Karyawan (Terima Cuti)'.  $karyawan[0]->name,
                'fitur' => 'presensi'
            ]);
