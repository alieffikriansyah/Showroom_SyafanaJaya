...
<div class="row align-items-center mb-4">
    <div class="col-12 col-md-7">
        <h3 class="font-weight-bold text-dark mb-1">Data Penjualan Mobil</h3>
    ...
    <div class="col-12 col-md-5 text-md-right mt-3 mt-md-0">
        ...
        <button type="button" class="btn btn-primary px-3 py-2 shadow-sm font-weight-bold" data-toggle="modal" data-target="#modaltambah">
            <i class="typcn typcn-plus mr-1"></i> Tambah Penjualan
        </button>
        @endif
    </div>
</div>
    ...
    <div class="card-body p-3 p-md-4">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    @php $i=1; @endphp
                    @forelse($penjualan as $penj)
                    <tr>
                        <td>{{$i}}</td>
                        <td class="font-weight-bold text-dark">{{$penj->karyawan->user->name ?? '-'}}</td>
                        <td><span class="badge badge-pill badge-light border font-weight-bold">{{$penj->merk}}</span></td>
                        <td>{{$penj->jenis_mobil}}</td>
                        <td><span class="badge badge-info">{{$penj->unit}} Unit</span></td>
                        <td>Rp {{number_format($penj->harga, 0, ',', '.')}}</td>
                        <td class="font-weight-bold text-success">Rp {{number_format($penj->harga * $penj->unit, 0, ',', '.')}}</td>
                        <td>{{date('d M Y', strtotime($penj->tanggal_penjualan))}}</td>
                        @if (!Auth::user()->karyawan)
                        <td class="text-center">
                            <button type="button" class="btn btn-sm btn-outline-warning btn-edit px-2 py-1" data-toggle="modal" data-target="#modalubah" data-id="{{$penj->id_penjualan}}">
                                <i class="typcn typcn-edit"></i>
                            </button>
                            <button type="button" class="btn btn-sm btn-outline-danger btn-delete px-2 py-1 ml-1" data-id="{{$penj->id_penjualan}}">
                    ...
                    <tr>
                        <td colspan="9" class="text-center text-muted py-5">
                            <i class="typcn typcn-shopping-bag d-block mb-2" style="font-size: 2rem;"></i>
